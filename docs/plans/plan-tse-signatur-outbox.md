...
- **Komponenten**:
  ...
  - *Störungsprotokoll-Komponente*: idempotentes Öffnen/Schließen von Zeiträumen; höchstens ein Zeitraum ist aktiv (Öffnen bei aktivem Zeitraum ist ein No-Op), und jeder Schreiber schließt nur Zeiträume seiner Grund-Art — ein Vorfall erscheint als ein Zeitraum, in der Aufholphase gefolgt von einem Rückstands-Zeitraum, nie als überlappendes Paar. Schreiber: Rückstands-Watchdog (Ticker neben dem Worker, prüft im Tick-Intervall das Alter des ältesten offenen Auftrags und öffnet/schließt den Rückstands-Zeitraum; dokumentiert auch einen hängenden Worker und hängt nicht am Leser-Traffic), Worker (TSE-weiter Fehler / erste erfolgreiche Signatur), Settings (fehlende Konfiguration / Einrichtung). Alle Lesepfade bleiben rein lesend.
...
- [ ] Fiskalische Projektion tabellengetrieben getestet: je Event-Typ signaturpflichtig ja/nein, processType, processData inkl. Vorzeichen-/Faktor-Fällen und datenabhängiger Sitzungseröffnung (mit/ohne Anfangsbestand).
...
- [ ] DSFinV-K-Export liest nur die Auftragstabelle (LEFT JOIN: kein Auftrag = nicht signaturpflichtig, keine Projektion zur Lesezeit) und füllt das Fehlerfeld für unsignierte Vorgänge; Verprobung TSE-Export gegen Export ohne Waisen.
