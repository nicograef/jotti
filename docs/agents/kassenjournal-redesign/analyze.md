...
- **Soll:** Tisch-Session (Abrechnungskreis) als Aggregat im Kasse-Kontext mit Subject `kassensitzung-{nr}/tisch-{id}`, Event-Typen `{event}:v1` ohne `tisch.`-Präfix
...
- Subject-Format: Alle Referenzen von `tisch:{id}` auf `kassensitzung-{nr}/tisch-{id}` aktualisieren
...
- Subject-Parsing: `tisch:{id}` → `kassensitzung-{nr}/tisch-{id}`
...
- Schema: `z_nr INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY`, `datum DATE`, `bezeichnung TEXT`, `status TEXT CHECK(...)`, `created_at`, `updated_at`
...
- **Zeilen ~160–473 (Events):** Komplett neu schreiben:
  ...
  - Subject-Format: `'tisch:1'` → `'kassensitzung-1/tisch-1'` (etc.)
...
- `InsertKassensitzung`: INSERT neue Kassensitzung
- `UpdateKassensitzungStatus`: UPDATE Status
- `GetOffeneKassensitzung`: WHERE status = 'offen'
- `GetKassensitzungByZNr`: Exakter Lookup
...
- Neu: Subject wird vom Application Service übergeben, Format `kassensitzung-{nr}/tisch-{tischID}`
...
- Subject-Konstruktion: `"tisch:" + strconv.Itoa(tischID)` → `"kassensitzung-" + strconv.Itoa(ksZNr) + "/tisch-" + strconv.Itoa(tischID)`
...
- `parseTischName` (Zeile ~89): `tisch:{id}` → neues Subject-Parsing für `kassensitzung-{nr}/tisch-{id}`
...
Das neue Subject-Format `kassensitzung-{nr}/tisch-{tischID}` erfordert, dass der Application Service die z_nr der offenen Kassensitzung kennt. Die z_nr kommt aus `kassensitzungen`. Das bedeutet:

- Jeder Tisch-Command muss zuerst `GetOffeneKassensitzung()` aufrufen
- Die z_nr wird aus der CRUD-Entität gelesen und ins Subject eingebaut
- **Frage:** ~~Wird das Datum als `YYYYMMDD`-String gespeichert oder aus einem `DATE`-Feld formatiert?~~ Gelöst: z_nr (int) wird direkt verwendet
...
Beim Schreiben eines Tisch-Events muss die `kassensitzung_nr` mitgegeben werden. Diese kommt aus `kassensitzungen`. Das bedeutet:

- Der Application Service liest `kassensitzungen` VOR dem Event-Write
