...
## Abschnitt 0a: DB-Schema Redo (§7 Architektur-Review) ✅

Kontext: `docs/redesign.md` §7 ändert `kassensitzung_state` von CQRS-Projektion zu `kassensitzungen` CRUD-Entität und das Subject-Format von `{YYYYMMDD}` zu `{nr}` mit `/`-Separator.

### Tasks

- [x] `kassensitzung_state`-Block entfernt und durch `kassensitzungen` CRUD-Entität ersetzt (`z_nr INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY`, kein `subject`, kein `last_event_id`, kein `last_event_version`)
- [x] FK auf `kassenjournal.kassensitzung_nr → kassensitzungen(z_nr)` hinzugefügt
- [x] FK auf `tisch_sessions.kassensitzung_nr → kassensitzungen(z_nr)` hinzugefügt
- [x] Subject-Kommentar auf neues Format aktualisiert (`kassensitzung-{nr}/tisch-{id}`)

---

## Abschnitt 0b: SQL-Queries Redo (§7 Architektur-Review) ✅

### Tasks

- [x] `backend/sqlc/queries/kassensitzung_state.sql` gelöscht
- [x] `backend/sqlc/queries/kassensitzungen.sql` erstellt (InsertKassensitzung, UpdateKassensitzungStatus, GetOffeneKassensitzung, GetKassensitzungByZNr, GetNextZNr, DeleteAllKassensitzungen)
- [x] `make sqlc` erfolgreich ausgeführt — generierter Code regeneriert

---

## Abschnitt 0c: Seed-Daten Redo (§7 Architektur-Review) ✅

`database/seed.sql` ist NICHT betroffen — nutzt altes `events`-Tabellenformat (Pre-Redesign) und enthält keine `kassensitzung_state`-Referenzen. Die Seed-Daten-Anpassung in Abschnitt 8 ist auf `kassensitzungen` (statt `kassensitzung_state`) aktualisiert.

---

## Abschnitt 0d: Dokumentation Redo (§7 Architektur-Review) ✅

### Tasks

- [x] `docs/redesign.md` §3.1–§3.13, §4, §5, §6 aktualisiert (§7 NICHT geändert)
- [x] `docs/handbuch.md` — ~20 Stellen aktualisiert
- [x] `docs/language.md` — ~6 Stellen aktualisiert
- [x] `docs/anforderungen.md` — 3 Stellen aktualisiert
- [x] `docs/diagrams.md` — ~19 Stellen aktualisiert (inkl. ER-Diagramm)
- [x] `docs/adr/event-sourcing.md` — ~15 Stellen aktualisiert
- [x] `docs/adr/cqrs.md` — ~12 Stellen aktualisiert
- [x] `docs/roadmap.md` — 1 Stelle aktualisiert
- [x] `.github/instructions/event-sourcing.instructions.md` — 6 Stellen aktualisiert
- [x] `.github/instructions/database.instructions.md` — 1 Stelle aktualisiert
- [x] `AGENTS.md` — 2 Stellen aktualisiert
- [x] `README.md` — 1 Stelle aktualisiert

---

## Abschnitt 1: DB-Schema — `kassenjournal`, `tisch_sessions`, `kassensitzung_state` ✅
...
- `docs/redesign.md:515-537` — neues `tisch_sessions`-Schema
...
- [x] Tabelle `table_state` durch `tisch_sessions` ersetzen: PK `subject TEXT` statt `tisch_id INTEGER`, neue Spalten `tisch_id INT NOT NULL REFERENCES tische(id)`, `kassensitzung_nr INT NOT NULL`, FK `last_event_id REFERENCES kassenjournal(id)`, Index `idx_tisch_sessions_ks_nr ON tisch_sessions(kassensitzung_nr)`
...
- [x] Datei `backend/sqlc/queries/table_state.sql` löschen und neue Datei `backend/sqlc/queries/tisch_sessions.sql` erstellen mit: `UpsertTischSession` (INSERT/ON CONFLICT auf `subject`, Spalten: subject, tisch_id, kassensitzung_nr, saldo_cents, unbezahlte_positionen, ausstehende_positionen, gesamt_zahlungen_cents, last_event_id, last_event_version), `GetTischSession` (WHERE subject = $1), `GetTischSessionsByKassensitzungNr` (WHERE kassensitzung_nr = $1, für Tischübersicht), `DeleteAllTischSession` (DELETE FROM tisch_sessions)
...
- [x] Datei `backend/sqlc/queries/reporting.sql` aktualisieren: Alle `FROM events` → `FROM kassenjournal`; alle Event-Typen `'tisch.bestellung-aufgenommen:v1'` → `'bestellung-aufgenommen:v1'` (analog für zahlung-kassiert, stornierung-erteilt, ausgabe-bestaetigt, auszahlung-geleistet); alle `WHERE timestamp >= @von AND timestamp < @bis` → `WHERE kassensitzung_nr = @kassensitzung_nr`; alle `table_state` → `tisch_sessions`; Subject-Parsing `SPLIT_PART(e.subject, ':', 2)::integer` → `tss.tisch_id` via JOIN auf `tisch_sessions tss ON tss.subject = e.subject`; Query-Parameter von `von`/`bis` auf `kassensitzung_nr` umstellen; `GetEigeneUebersicht` ebenfalls auf `kassenjournal` + `kassensitzung_nr` umstellen
...
- [x] Datei `backend/sqlc/queries/tables.sql` aktualisieren: `GetAktiveTische` — JOIN auf `tisch_sessions` statt `table_state`, mit zusätzlichem Parameter `kassensitzung_nr` (`LEFT JOIN tisch_sessions tss ON tss.tisch_id = t.id AND tss.kassensitzung_nr = $1`); `GetAktiveTischeMitFavoriten` — gleiches Pattern mit `kassensitzung_nr`-Parameter (Position nach user_id-Parameter)
...
- [x] Datei `backend/domain/kasse/stream_type.go` erstellen: `StreamType`-Typ als `string` mit zwei Konstanten `StreamTypeKassensitzung StreamType = "kassensitzung"` und `StreamTypeTischSession StreamType = "tisch-session"`
- [x] Datei `backend/domain/kasse/tisch_session_events.go` erstellen: Event-Typ-Konstanten **ohne** `tisch.`-Präfix (`EventTypeBestellungAufgenommenV1 = "bestellung-aufgenommen:v1"` etc. für alle 5 Tisch-Events); Event-Data-Structs (`bestellungAufgenommenV1Data`, `zahlungKassiertV1Data`, `stornierungErteiltV1Data`, `ausgabeBestaetigtV1Data`, `auszahlungGeleistetV1Data`) mit `json`-Tags — **Vorlage:** bestehende Structs aus `domain/table/bestellungAufgenommenEvent.go` etc., identische Felder; Event-Erstellungsfunktionen (`NewBestellungAufgenommenEvent(subject string, ...)` — Subject wird als Parameter übergeben, nicht mehr intern aus tischID konstruiert); `positionEventData`-Struct (JSON-Repräsentation einer Position für Events) aus `bestellung.go` hierher verschieben
- [x] Datei `backend/domain/kasse/kassensitzung_events.go` erstellen: 6 neue Event-Typ-Konstanten (`EventTypeKassensitzungEroeffnetV1 = "kassensitzung-eroeffnet:v1"`, `EventTypeAnfangsbestandGesetztV1`, `EventTypeKassenbewegungGebuchtV1`, `EventTypeKassensturzDurchgefuehrtV1`, `EventTypeDifferenzSollIstGebuchtV1`, `EventTypeTagesabschlussErstelltV1`); Event-Data-Structs mit `json`-Tags gemäß redesign.md §3.6 (`kassensitzungEroeffnetV1Data{Datum, Bezeichnung, EroeffnetVon}`, `anfangsbestandGesetztV1Data{BetragCents, GesetztVon}`, `kassenbewegungGebuchtV1Data{BewegungID, Art, BetragCents, Kommentar, GebuchtVon}`, `kassensturzDurchgefuehrtV1Data{SollBestandCents, IstBestandCents, DifferenzCents, DurchgefuehrtVon}`, `differenzSollIstGebuchtV1Data{BetragCents, GebuchtVon}`, `tagesabschlussErstelltV1Data{ZNr, ZeitraumVon, ZeitraumBis, UmsatzGesamtCents, StornierungCents, AuszahlungenCents, GeldtransitCents, ErstelltVon}`); Event-Erstellungsfunktionen für jeden Typ (`NewKassensitzungEroeffnetEvent(subject string, userID int, userName string, data)` etc.)
- [x] Datei `backend/domain/kasse/tisch_session.go` erstellen: `TischSession`-Struct (Subject, TischID, KassensitzungNr, SaldoCents, UnbezahltePositionen, AusstehendePositionen, GesamtZahlungenCents, LastEventID, LastEventVersion) — **Vorlage:** `TischState` aus `projection.go`, erweitert um Subject, TischID, KassensitzungNr; `ApplyEvent(state *TischSession, e event.Event)` — identische Logik wie bisheriges `ApplyEvent` aus `projection.go`; `ComputeNichtStorniertePositionen(events []event.Event)` — identisch mit bisherigem Code
- [x] Datei `backend/domain/kasse/kassensitzung.go` erstellen: `KassensitzungState`-Struct (ZNr, Datum, Bezeichnung, Status, CreatedAt, UpdatedAt); Status-Konstanten `KassensitzungStatusOffen = "offen"`, `KassensitzungStatusAbgeschlossen = "abgeschlossen"`
- [x] Datei `backend/domain/kasse/subject.go` erstellen: Helper-Funktionen für Subject-Konstruktion und -Parsing: `KassensitzungSubject(zNr int) string` → `"kassensitzung-" + strconv.Itoa(zNr)`; `TischSessionSubject(zNr int, tischID int) string` → `"kassensitzung-" + strconv.Itoa(zNr) + "/tisch-" + strconv.Itoa(tischID)`; `ParseTischIDFromSubject(subject string) (int, error)` — extrahiert tischID aus `kassensitzung-{nr}/tisch-{id}` (Segment nach `/tisch-`); `ParseZNrFromSubject(subject string) (int, error)` — extrahiert z_nr aus Subject
- [x] Datei `backend/domain/kasse/bestellung.go` erstellen: `Position`-Struct (PositionID, VarianteID, ProduktName, VarianteName, Kategorie, Einzelpreis, Menge) — identisch mit bisherigem `table.Position`; `Bestellung`-Struct (ID, UserID, TischID, Positionen, GesamtPreisCents, Kommentar, AufgenommenAm); `PositionRef`-Struct (PositionID, Menge); Zod-Schemas für Bestellung/Position — **Vorlage:** `domain/table/bestellung.go`, 1:1 kopieren
- [x] Datei `backend/domain/kasse/zahlung.go` erstellen: `Zahlung`-Struct — Vorlage: `domain/table/zahlung.go`
- [x] Datei `backend/domain/kasse/stornierung.go` erstellen: `Stornierung`-Struct — Vorlage: `domain/table/stornierung.go`
- [x] Datei `backend/domain/kasse/ausgabe.go` erstellen: `Ausgabe`-Struct — Vorlage: `domain/table/ausgabe.go`
- [x] Datei `backend/domain/kasse/auszahlung.go` erstellen: `Auszahlung`-Struct — Vorlage: `domain/table/auszahlung.go`
- [x] Datei `backend/domain/kasse/historie.go` erstellen: `HistorieEintrag`-Struct und `GetHistoryFromEvents(events []event.Event)` — Vorlage: `domain/table/events.go:34-133` (Funktion `GetHistoryFromEvents` + `accumulatePositionen` + `reduceByPosition`); Event-Typ-Referenzen auf neue Konstanten ohne `tisch.`-Präfix aktualisieren

#### 3b: `domain/table/` auf Stammdaten reduzieren

- [x] Alle Event-Sourcing-Dateien aus `backend/domain/table/` löschen: `events.go`, `events_test.go`, `projection.go`, `projection_test.go`, `bestellungAufgenommenEvent.go`, `zahlungKassiertEvent.go`, `stornierungErteiltEvent.go`, `ausgabeBestaetigtEvent.go`, `auszahlungGeleistetEvent.go`, `bestellung.go`, `zahlung.go`, `stornierung.go`, `ausgabe.go`, `auszahlung.go` (nur `tisch.go` bleibt)
- [x] In `backend/domain/table/tisch.go` anpassen: `AktiverTisch` und `AktiverTischMitFavorit` bleiben (SaldoCents kommt jetzt aus `tisch_sessions`, aber das Feld bleibt gleich); sicherstellen, dass keine Imports auf gelöschte Dateien verweisen; ggf. nicht mehr benötigte Imports entfernen

#### 3c: Tests für `domain/kasse/`

- [x] Datei `backend/domain/kasse/tisch_session_test.go` erstellen: Tests für `ApplyEvent` — Vorlage: `domain/table/projection_test.go` (alle Test-Cases übernehmen, Event-Typen auf neue Konstanten anpassen, Subject-Format auf `kassensitzung-1/tisch-42`); Tests für `ComputeNichtStorniertePositionen`
- [x] Datei `backend/domain/kasse/historie_test.go` erstellen: Tests für `GetHistoryFromEvents` — Vorlage: `domain/table/events_test.go` (Event-Typen + Subjects anpassen)
- [x] Datei `backend/domain/kasse/subject_test.go` erstellen: Tests für `KassensitzungSubject`, `TischSessionSubject`, `ParseTischIDFromSubject`, `ParseZNrFromSubject`
...
- [x] Datei `backend/repository/kassenjournal_repo/repo.go` erstellen: **Vorlage:** `event_repo/repo.go`. Struct `Repo` mit `db *sql.DB`. Interface-Methoden:
  ...
  - `ReadTischSession(ctx, subject string)` — liest `tisch_sessions` nach Subject (statt `table_state` nach tischID). Gibt Zero-Value zurück wenn kein Eintrag existiert.
  - `GetOffeneKassensitzung(ctx)` — liest `kassensitzungen` WHERE status='offen'
...
- [x] §8 Read Models aktualisieren: `tisch_sessions` (PK: subject, session-scoped) + `kassensitzung_state`; Tischübersicht als JOIN-Query; neue Projektionstabellen dokumentieren
...
- [x] Namenskonventionen aktualisieren: Go-Structs `domain/kasse/`, DB-Tabellen `kassenjournal`, `tisch_sessions`, `kassensitzung_state`
...
- [x] Diagramm 17 (DB-Schema ER): `kassenjournal`, `kassensitzung_state`, `tisch_sessions`
...
- [x] `docs/adr/event-sourcing.md`: Event-Typen ohne Präfix; `events` → `kassenjournal`; `table_state` → `tisch_sessions`; Subject-Format; neue KS-Events
