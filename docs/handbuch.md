...
| Bereich   | Seiten                                                                                                                                                                                                                                                                                         |
| --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Service   | Tischübersicht → Tisch-Detail (Tabs: Bestellen, Kassieren, Historie). Stornieren ist für `serviceleitung`/`admin` im Historie-Tab verfügbar. Direktverkauf (eigene Seite, Tabs: Verkaufen, Historie) bestellt, kassiert und gibt in einem Schritt aus, ohne Tisch.                             |
...
Frontend und Backend werden gemeinsam ausgeliefert; ein Client mit altem Bundle spricht nach einem Update gegen ein neues Backend. Statt Kompatibilität über Releases hinweg zu garantieren, erzwingt das Frontend das Neuladen — **Antwortformate dürfen sich deshalb weiterhin frei ändern** (siehe Freeze-Disziplin in `AGENTS.md`), solange der Handshake intakt bleibt. Der Reload ist dabei verzögert, nicht sofort: bis zu 30 s bis zur nächsten Abfrage, bei offenem Vorgang bis zu dessen Abschluss, im gebremsten Zustand gar nicht. In diesem Fenster spricht ein altes Bundle weiter gegen das neue Backend — ein geändertes Antwortformat muss dort ohne Datenschaden scheitern dürfen.

- **Erkennung:** `useVersion` (`hooks/use-version.ts`) pollt `POST /health` alle 30 s (`VERSIONSABFRAGE_INTERVALL_MS`) und holt die Abfrage beim Zurückkehren in den Vordergrund nach (kein `staleTime`). Solange keine Antwort vorliegt, ist die Version `undefined` und es passiert nichts; ein Fehlschlag ersetzt die zuletzt bekannte Version nicht (react-query behält sie) — ein Serverneustart ist damit kein Versionswechsel.
- **Vergleich:** `istVersionsabweichung` (`lib/version.ts`) meldet nur, wenn **beide** Seiten dem Release-Muster `v<major>.<minor>.<patch>` mit optionalem Suffix genügen und sich unterscheiden. Vorabversionen wie `v1.2.3-rc1` zählen mit und schalten den Handshake scharf; das Muster ist wortgleich mit der Prüfung im Makefile-Target `prod-up`. In Dev, E2E und Tests (`dev`, `dev-<sha>`) ist der Handshake still. Richtungslos: Ein Rollback auf eine ältere Serverversion zählt ebenso.
- **Vorgangs-Register:** `VorgangsRegisterSingleton` (`lib/VorgangsRegister.ts`) zählt offene Vorgänge — gefüllter Korb, getroffene Auswahl, laufende Mutation, angefangenes Formular. Angemeldet wird ausschließlich über `useOffenerVorgang(offen)`, das im Effekt-Cleanup abmeldet. Bei leerem Register lädt `useVersionsGuard` (`hooks/use-versions-guard.ts`) sofort neu; sonst wartet er, bis der letzte Vorgang abgeschlossen oder verworfen ist. Träger des Guards ist `VersionsHinweis` (`components/common/VersionsHinweis.tsx`, montiert in `App.tsx`, nicht wegklickbar, nicht-modal): Die Komponente zeigt nicht nur den Sperrhinweis, sie ist der einzige Aufrufer von `useVersionsGuard` — ohne sie läuft auch der unsichtbare Regelfall, der sofortige Reload, nicht.
