...
**DSFinV-K-Export:** Der Export (`/admin/export/dsfinvk`, Orchestrierung in `backend/api/export`, Mapper und Archiv in `backend/domain/dsfinvk`) bündelt Stammdaten-, Einzelaufzeichnungs- und Z-Bon-Daten als DSFinV-K-CSV mit der amtlichen `index.xml` und DTD zu einem ZIP; nachsignierte Vorgänge werden beim Mappen mit der Signatur-Seitentabelle vereinigt. Datei-Struktur und Pflichtfelder → [compliance.md §6](compliance.md#6-dsfinv-k-export-schnittstelle).
