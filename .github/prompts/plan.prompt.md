---
description: "Erstellt eine plan.md mit kleinteiliger Task-Liste aus einer bestehenden analyze.md. Für iteratives Abarbeiten durch einen Coding Agent."
agent: "agent"
argument-hint: "Pfad zur analyze.md (z.B. docs/agents/theory-cleanup/analyze.md)"
---

# Planungsdatei erstellen

Erzeuge aus der referenzierten Analyse eine **plan.md** im selben Verzeichnis. Die Datei enthält eine kleinteilige Task-Liste im Checkbox-Style, die ein Coding Agent Abschnitt für Abschnitt abarbeiten kann.

## Vorgehen

1. **Lies** die referenzierte analyze.md vollständig
2. **Leite Implementierungsschritte ab** — beschreibe das Was UND das Wie: nicht nur "erstelle Handler", sondern welche Funktion, welche Parameter, welches Pattern (mit Verweis auf existierenden Code als Vorlage aus der analyze.md)
3. **Unterteile** jeden Implementierungsschritt in möglichst kleine, atomare Tasks
4. **Gruppiere** die Tasks in logische Abschnitte (z.B. nach Schicht: Domain, Repository, Handler, Frontend)
5. **Definiere pro Abschnitt den Kontext** — liste für jeden Abschnitt exakt auf, welche Dateien (mit Zeilennummern aus der analyze.md) der Agent lesen muss, bevor er mit der Arbeit beginnt. Nur das Nötigste für diesen Abschnitt, nicht alles.
6. **Analysiere Abhängigkeiten** zwischen Abschnitten: Welche Abschnitte arbeiten an denselben Dateien? Welche sind voneinander unabhängig?
7. **Schreibe** die plan.md mit Parallelisierungs-Hinweisen
...
- `pfad/andere.ts:100-120` — <warum relevant>
