...
- Every phase names: **Depends on**, **Modell** (Opus or Sonnet per the CLAUDE.md routing: Sonnet for mechanical, clearly specified work; Opus for implementation, subtle correctness, architecture), **Review-Tier** (a Fable sweep followed by an Opus review per phase is the standard for this run), **Gate** (the exact command(s) that prove the phase: make check, make verify, make website-check, make test-e2e, a specific go test/vitest invocation, or a CI grep), and testable acceptance criteria that each name their own change (implement-plan commits one criterion at a time).
...
- Known pre-existing drift the lead already collected (include as criteria in the matching phase if the findings document lists them; otherwise add them to the matching docs phase): handbuch.md says "drei" Fehlversuche where the code (MaxDruckversuche = 6) and README say "sechs"; handbuch.md names DruckerConfigPage instead of DruckstationConfigPage; docs/adrs/README.md and docs/adrs/04_warn-bestaetigung.md not prettier-clean; e2e files not prettier-clean; scripts/setup-dev-tools.sh does not rebuild golangci-lint on a toolchain-only change; TERMS.md "setzt um" wording (owner decision; Open question, not a criterion); PREVIOUS_VERSION v0.17.1 vs v0.17.3 belongs to the release phase, not this plan.
