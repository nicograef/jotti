export const meta = {
  ...
  description:
    'Full-repo multi-expert review of jotti (every code, doc and config file): one Fable sweep per area hands over to Opus reviewers, adversarial Opus verification and a consolidated findings document',
  phases: [
    {
      title: 'Sweep',
      detail: 'one Fable sweep per area → hand-over brief',
      model: 'fable',
    },
    {
      title: 'Review',
      detail: '22 units × 3 lenses + 8 cross-layer flows',
      model: 'opus',
    },
    {
      title: 'Verify',
      detail: 'refuters for blocker/major findings',
      model: 'opus',
    },
    {
      title: 'Consolidate',
      detail: 'per-area consolidation and findings document',
      model: 'opus',
    },
  ],
}
...
const HANDBOOK =
  ...
  (A.handbook
    ? `${A.handbook}/cleanup`
    : '/home/user/handbook/.claude/skills/cleanup')
...
const ONLY_AREA = A.area || null
const SECTIONS_DIR = A.sectionsDir || null
const ASSEMBLE_FROM = A.assembleFrom || null
...
const ALL_UNITS = [
  ...
  { key: 'domain-kasse', kind: 'go', paths: ['backend/domain/kasse'] },
  {
    ...
    paths: [
      'backend/domain/tse',
      'backend/domain/steuer',
      'backend/domain/event',
      'backend/domain/jwt',
      'backend/domain/user',
      'backend/domain/produkt',
      'backend/domain/druckstation',
      'backend/domain/tisch',
      'backend/domain/betreiber',
  ...
  },
  {
    ...
    kind: 'go',
    paths: [
      'backend/repository/tse_repo',
      'backend/repository/kassenjournal_repo',
      'backend/repository/druckauftrag_repo',
      'backend/repository/kassensitzungen_repo',
    ],
  },
  {
    key: 'repo-stammdaten',
    kind: 'go',
    paths: [
      'backend/repository/produkt_repo',
      'backend/repository/user_repo',
      'backend/repository/tisch_repo',
      'backend/repository/druckstation_repo',
      'backend/repository/betreiber_repo',
      'backend/repository/favorit_repo',
      'backend/repository/reporting_repo',
    ],
  },
  {
    key: 'backend-infra',
    ...
    paths: [
      ...
      'backend/db',
      'backend/seed',
      'backend/dsfinvkpruefung',
      'backend/sqlc/queries',
      'backend/sqlc.yaml',
      'backend/*.go',
      'backend/go.mod',
      'backend/Dockerfile',
  ...
  {
    key: 'fe-service-components',
    kind: 'ts',
    paths: ['frontend/src/service/components'],
  },
  {
    key: 'fe-service-rest',
    kind: 'ts',
    paths: [
      ...
      'frontend/src/service/table',
      'frontend/src/service/direktverkauf',
      'frontend/src/service/product',
    ],
  },
  {
    key: 'fe-admin-fiscal',
    kind: 'ts',
    paths: [
      'frontend/src/admin/reporting',
      'frontend/src/admin/kasse',
      'frontend/src/admin/tse',
      'frontend/src/admin/finanzamt',
    ],
  },
  {
    key: 'fe-admin-stammdaten',
    kind: 'ts',
    paths: [
      'frontend/src/admin/users',
      'frontend/src/admin/products',
      'frontend/src/admin/tables',
      'frontend/src/admin/settings',
...
const ALL_AREAS = {
  ...
  'Cross-Layer-Flüsse': ALL_FLOWS.map((_, i) => `flow-${i + 1}`),
}
const AREA_ORDER = Object.keys(ALL_AREAS)
const slug = (area) =>
  area
    .toLowerCase()
    .replace(/ä/g, 'ae')
...
if (ONLY_AREA && !ALL_AREAS[ONLY_AREA])
  throw new Error(
    `Unbekannter Bereich: ${ONLY_AREA} (erlaubt: ${AREA_ORDER.join(', ')})`,
  )
if (SECTIONS_DIR && !ONLY_AREA)
  throw new Error(
    'sectionsDir verlangt area: die Sektionsdatei trägt die Statistik eines Bereichs',
  )
const AREAS = ONLY_AREA ? { [ONLY_AREA]: ALL_AREAS[ONLY_AREA] } : ALL_AREAS
const UNITS = ALL_UNITS.filter((u) =>
  Object.values(AREAS).some((units) => units.includes(u.key)),
)
const FLOWS = !ONLY_AREA || ONLY_AREA === 'Cross-Layer-Flüsse' ? ALL_FLOWS : []
const areaOfUnit = (key) => AREA_ORDER.find((a) => ALL_AREAS[a].includes(key))
      ...
      items: {
        ...
        properties: {
          ...
          category: {
            type: 'string',
            description:
              'boundary-consistency | correctness | security | readability | principle | code-smell | architecture | docs-accuracy | convention | test-quality | ops | large-refactor',
          },
          file: { type: 'string' },
          ...
          what: {
            type: 'string',
            description:
              'rule or defect, reference the cleanup reference file and rule name where applicable',
          },
          ...
          proof: {
            type: 'string',
            description:
              'quoted code/doc lines or command output that shows it',
          },
        },
        required: [
          'severity',
...
const BRIEF = {
  ...
  properties: {
    hotspots: {
      ...
      items: {
        ...
        properties: {
          ...
          lens: {
            type: 'string',
            description: 'cleanup | correctness | conventions | cross-layer',
          },
        },
        required: ['file', 'why', 'lens'],
      },
    },
    ...
    readingOrder: {
      type: 'array',
      items: { type: 'string' },
      description: 'files or directories the deep reviewers should read first',
    },
  ...
  required: [
    'hotspots',
    'suspectedDefectClasses',
    'readingOrder',
    'questionsForReviewers',
    'filesSkimmed',
    'summary',
  ],
}
...
const SECTION = {
  ...
  required: ['markdown', 'defectClasses', 'top'],
}

const headerText = (stats) => `# Findings: Vollreview jotti (${DATE})

> Quelle: Multi-Experten-Review aller Dateien des Repos (Stand \`${BRANCH}\` @ ${REV}),
> ${ALL_UNITS.length} Einheiten × 3 Linsen (Cleanup-Skill, Korrektheit/Security, Konventionen/Doku) plus
...
| Kennzahl | Wert |
| --- | --- |
| Gelesene Dateien (laut Reviewern) | ${stats.filesReviewed} |
| Rohbefunde | ${stats.raw} |
| Nach Dedupe | ${stats.deduped} |
| Verifiziert | ${stats.verified} |
| Verworfen | ${stats.dropped} |
| Verbleibend | ${stats.remaining} (Blocker ${stats.counts.blocker}, Major ${stats.counts.major}, Minor ${stats.counts.minor}) |
| Reviewer ohne Ergebnis | ${stats.failedReviewers} |
`
...
if (ASSEMBLE_FROM) {
  ...
  const placeholder = {
    ...
    dropped: '{{dropped}}',
    remaining: '{{remaining}}',
    counts: { blocker: '{{blocker}}', major: '{{major}}', minor: '{{minor}}' },
...
phase('Sweep')
log(
  `Sweep: ${Object.keys(AREAS).length} Fable-Sweep(s) (je Bereich), dann Übergabe an Opus`,
)
const briefs = {}
const sweepResults = await parallel(
  Object.entries(AREAS).map(([area, units]) => () => {
    const scope =
      area === 'Cross-Layer-Flüsse'
        ? `the eight cross-layer flows:\n${ALL_FLOWS.map((f, i) => `${i + 1}. ${f}`).join('\n')}`
        : `paths (git ls-files globs, relative to ${REPO}): ${ALL_UNITS.filter(
            (u) => units.includes(u.key),
          )
            .flatMap((u) => u.paths)
            .join(', ')}`
    return agent(
...
log(
  `Sweep fertig: ${sweepResults.filter(Boolean).length}/${Object.keys(AREAS).length} Briefs`,
)

phase('Review')
log(
  `Review: ${UNITS.length} Einheiten × 3 Linsen + ${FLOWS.length} Cross-Layer-Flüsse (Opus)`,
)
const reviewItems = []
for (const u of UNITS)
  for (const lens of Object.keys(LENSES)) reviewItems.push({ u, lens })
...
reviews.forEach((r, i) => {
  if (r)
    r.findings.forEach((f) =>
      raw.push({ ...f, unit: reviewItems[i].u.key, lens: reviewItems[i].lens }),
    )
})
flows.forEach((r, i) => {
  if (r)
    r.findings.forEach((f) =>
      raw.push({ ...f, unit: `flow-${i + 1}`, lens: 'cross-layer' }),
    )
})
const filesReviewed = [...reviews, ...flows]
  .filter(Boolean)
  .reduce((a, r) => a + (r.filesReviewed || 0), 0)
const failedReviewers =
  reviews.filter((r) => !r).length + flows.filter((r) => !r).length
const failedUnits = reviewItems
  .filter((_, i) => !reviews[i])
  .map(({ u, lens }) => `${u.key}:${lens}`)
  .concat(flows.map((r, i) => (r ? null : `flow-${i + 1}`)).filter(Boolean))
...
for (const f of raw) {
  ...
  if (!seen.has(key)) seen.set(key, f)
    ...
    if (rank[f.severity] > rank[prev.severity])
      seen.set(key, { ...f, alsoFrom: prev.lens })
  }
}
const deduped = [...seen.values()]
log(
  `Review fertig: ${raw.length} Rohbefunde, ${deduped.length} nach Dedupe, ${filesReviewed} Dateien gelesen, ${failedReviewers} Reviewer ohne Ergebnis${failedUnits.length ? ` (${failedUnits.join(', ')})` : ''}`,
)
...
const verifyList = capped
  ? toVerify
      .sort(
        (a, b) =>
          (a.severity === 'blocker' ? -1 : 1) -
          (b.severity === 'blocker' ? -1 : 1),
      )
      .slice(0, CAP)
  : toVerify
if (capped)
  log(
    `Verifikation gekappt: ${verifyList.length} von ${toVerify.length} Blocker/Major-Befunden werden geprüft (Blocker zuerst); der Rest bleibt „unverifiziert"`,
  )
  ...
  verifyList.map((f, idx) => () => {
    ...
    return parallel(
      ...
      const stands =
        f.severity === 'blocker'
          ? notRefuted >= 2
          : notRefuted === v.length && v.length > 0
      return {
        ...f,
        verified: stands ? 'bestätigt' : 'verworfen',
        votes: v.map((x) => x.reason),
      }
    })
  }),
)
verified
  .filter(Boolean)
  .forEach((f) =>
    verifiedSet.set(`${f.file}:${f.lines}:${f.what.slice(0, 40)}`, f),
  )
const finalFindings = deduped
  .map((f) => {
...
const dropped = verified
  ...
  .filter((f) => f.verified === 'verworfen')
log(
  `Verifikation: ${verified.filter(Boolean).length} geprüft, ${dropped.length} verworfen, ${finalFindings.length} Befunde bleiben`,
)
...
const sections = await parallel(
  Object.entries(AREAS).map(([area, units]) => () => {
    ...
    const writeInstruction = SECTIONS_DIR
      ? `\n\nAfter composing, WRITE two files (create ${SECTIONS_DIR} if missing): ${SECTIONS_DIR}/${slug(area)}.md containing exactly the markdown section, and ${SECTIONS_DIR}/${slug(area)}.json containing {"area": "${area}", "defectClasses": [...], "top": [...]} plus the stats given below verbatim. Return the same markdown/defectClasses/top in the structured output as well.\nSTATS (copy verbatim into the JSON): ${JSON.stringify(
          {
            filesReviewed,
            raw: raw.length,
            ...
            remaining: finalFindings.length,
            counts: finalFindings.reduce(
              (c, f) => {
                c[f.severity]++
                return c
              },
...
if (ONLY_AREA) {
  log(
    `Bereich ${ONLY_AREA} fertig: ${stats.remaining} Befunde (Blocker ${counts.blocker}, Major ${counts.major}, Minor ${counts.minor})`,
  )
  return {
    area: ONLY_AREA,
    ...stats,
    sectionWritten: !!SECTIONS_DIR,
    section: sections[0]
      ? { defectClasses: sections[0].defectClasses, top: sections[0].top }
      : null,
  }
}
...
return { ...stats, outFile: OUT, assembled }
