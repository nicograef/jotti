...
func mustCreateOrderEvent(t *testing.T, userID, tableID int, products []Position) e.Event {
	t.Helper()
	event, err := NewBestellungAufgenommenEvent(userID, "TestUser", tableID, products, "")
	if err != nil {
...
func mustCreatePaymentEvent(t *testing.T, userID, tableID int, positions []Position, gesamtZahlungCents int) e.Event {
	t.Helper()
	event, err := NewZahlungKassiertEvent(userID, "TestUser", tableID, positions, gesamtZahlungCents, "")
	if err != nil {
