...
func TestCreateTable(t *testing.T) {
	...
	repo := repository.NewMock([]domain.Table{}, nil)
...
func TestCreateTable_Error(t *testing.T) {
	repo := repository.NewMock([]domain.Table{}, db.ErrAlreadyExists)
	command := Command{TableRepo: repo}
...
func TestUpdateTable(t *testing.T) {
	repo := repository.NewMock([]domain.Table{{ID: 1, Name: "Old Name", Status: domain.ActiveStatus}}, nil)
	command := Command{TableRepo: repo}
...
func TestUpdateTable_NotFound(t *testing.T) {
	repo := repository.NewMock([]domain.Table{}, db.ErrNotFound)
	command := Command{TableRepo: repo}
...
func TestActivateTable(t *testing.T) {
	repo := repository.NewMock([]domain.Table{{ID: 1, Name: "Table 1", Status: domain.InactiveStatus}}, nil)
	command := Command{TableRepo: repo}
	...
	table, err := repo.GetTable(context.Background(), 1)
	if err != nil {
		t.Fatalf("expected no error retrieving table, got %v", err)
	}
	if table.Status != domain.ActiveStatus {
		t.Errorf("expected table status to be Active, got %v", table.Status)
	}
}
...
func TestDeactivateTable(t *testing.T) {
	repo := repository.NewMock([]domain.Table{{ID: 1, Name: "Table 1", Status: domain.ActiveStatus}}, nil)
	command := Command{TableRepo: repo}
	...
	if table.Status != domain.InactiveStatus {
		t.Errorf("expected table status to be Inactive, got %v", table.Status)
	}
}
