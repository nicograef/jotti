...
func TestTischErstellen(t *testing.T) {
	...
	repo := table_repo.NewMock([]table.Tisch{}, nil)
...
func TestTischAktualisieren(t *testing.T) {
	repo := table_repo.NewMock([]table.Tisch{{ID: 1, Name: "Old Name", Status: table.ActiveStatus}}, nil)
	command := Command{TableRepo: repo}
...
func TestTischAktivieren(t *testing.T) {
	repo := table_repo.NewMock([]table.Tisch{{ID: 1, Name: "Tisch 1", Status: table.InactiveStatus}}, nil)
	command := Command{TableRepo: repo}
