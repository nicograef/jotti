...
// Die vier fiskaly-Endpunkte sprechen synchron mit fiskaly und koennen die
// globale 10-Sekunden-Schreibfrist des Servers ueberschreiten. Ohne die
// verlaengerte Frist stirbt die Antwort auf der Verbindung — bei der
// Einrichtung samt PUK und Admin-PIN, die genau einmal ausgeliefert und
// nirgends persistiert werden. Der Test laeuft durch die LoggingMiddleware,
// weil sie in Produktion die gesamte Routenkette umschliesst (app/app.go) und
// die Frist auch durch ihren ResponseWriter-Wrapper hindurch ankommen muss.
//
...
// lange der fiskaly-Roundtrip zuvor gedauert hat.
func TestTSESetupHandler_VerlaengertSchreibfristVorErstemSchreibvorgang(t *testing.T) {
	...
	query := &QueryHandler{Query: &mockSettingsQuery{}}
	...
	faelle := []struct {
		...
		{"/admin/test-tse-verbindung", query.TestTSEVerbindungHandler(), `{}`},
		{"/admin/tse-setup-pruefen", query.CheckTSESetupHandler(), `{"apiKey":"key","apiSecret":"secret"}`},
	}
