...
func TestActivateProduct(t *testing.T) {
	inactiveProduct := testProduct
	inactiveProduct.Status = product.InactiveStatus

	repo := product_repo.NewMock([]product.Produkt{inactiveProduct}, nil)
	cmd := Command{ProductRepo: repo}

	err := cmd.ActivateProduct(context.Background(), 1)
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
	...
	if updated.Status != product.ActiveStatus {
		t.Errorf("expected product status Active, got %v", updated.Status)
	}
}
...
func TestActivateProduct_NotFound(t *testing.T) {
	...
	err := cmd.ActivateProduct(context.Background(), 999)
	if err != ErrProduktNotFound {
		t.Fatalf("expected ErrProduktNotFound, got %v", err)
	}
}
...
func TestDeactivateProduct(t *testing.T) {
	...
	err := cmd.DeactivateProduct(context.Background(), 1)
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
	...
	if updated.Status != product.InactiveStatus {
		t.Errorf("expected product status Inactive, got %v", updated.Status)
	}
}
...
func TestDeactivateProduct_NotFound(t *testing.T) {
	...
	err := cmd.DeactivateProduct(context.Background(), 999)
	if err != ErrProduktNotFound {
		t.Fatalf("expected ErrProduktNotFound, got %v", err)
	}
}
